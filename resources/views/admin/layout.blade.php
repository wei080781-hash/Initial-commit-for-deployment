    ...
    <main :class="sidebarOpen ? 'ml-64' : 'ml-20'" class="transition-all duration-300 min-h-screen">
        ...
        <header class="h-16 bg-white border-b border-stone-200 flex items-center justify-between px-8 sticky top-0 z-40">
            ...
            <div class="flex items-center space-x-6">
                <span class="text-stone-400 text-sm">{{ date('Y/m/d') }}</span>
                
                <!-- 前端驅動的使用者區域 -->
                <div id="adminUserArea" class="hidden flex items-center space-x-2 relative" x-data="{ open: false }">
                    <button @click="open = !open" class="flex items-center space-x-2 focus:outline-none">
                        <div id="userAvatar" class="w-8 h-8 rounded-full bg-orange-100 flex items-center justify-center text-orange-400 font-bold text-xs uppercase"></div>
                        <span id="userName" class="font-medium text-stone-700"></span>
                        <svg class="w-4 h-4 text-stone-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    <!-- 下拉選單 -->
                    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg py-2 border border-stone-100 z-50">
                        <button onclick="logout()" class="block w-full text-left px-4 py-2 text-sm text-stone-700 hover:bg-stone-100 transition">登出</button>
                    </div>
                </div>
                <a id="loginLink" href="{{ route('login') }}" class="text-orange-500 hover:underline">請先登入</a>
            </div>
